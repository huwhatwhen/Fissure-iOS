...
public class FissureGameScreen extends ScreenAdapter {
    ...
    public FissureGameScreen(TextureAtlas atlas) {
        ...
        for (int i = 0; i < NUM_TILES; i++) mIntegers.add(i);
    ...
    public void render(float delta) {
        if (Gdx.input.getInputProcessor().equals(mWorld) && delta < 0.1) {
            mElapsedTime += delta;
        }
    ...
    private boolean doAdCheck() {
        if (mIOSLink != null && mAdCounter >= 5) {
            mAdCounter = 0;
            mData.putInteger("adCounter", mAdCounter);
            mData.flush();
            mIOSLink.showInterstitial();
    ...
    private void doMinerDeathSequence() {
        ...
        /*
        if (mIOSLink != null) {
            //mIOSLink.showBanner(true);
        }
        */

        mScoreBG.mScore = mScore;

        if (mIOSLink != null && mScore.setScale(2, BigDecimal.ROUND_UP).floatValue() > mScoreBG.mHighScore.floatValue()) {
